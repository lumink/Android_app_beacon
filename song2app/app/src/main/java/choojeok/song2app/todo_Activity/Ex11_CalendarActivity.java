package choojeok.song2app.todo_Activity;
...
public class Ex11_CalendarActivity extends Activity implements OnClickListener,
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Date date = new Date();// Set TODAY's date
	...
	private void fillDate(int year, int mon) {
		...
		int day = current.getDay(); // store day in int type

		for (int i = 0; i < day; i++) {
			mItems.add("");
		}

		current.setDate(32);// change date 32 to date 1
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		...
		if (mItems.get(arg2).equals("")) {
			...
			Intent intent = new Intent(this, ExToday.class); // press that date
